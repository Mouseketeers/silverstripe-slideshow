...
class Slideshow extends DataObjectDecorator
{
    ...
    private function update_other_slideshows($data)
    {
        ...
        if ($data['UpdateSlideshows'] == 'section') {
            $filter .=  ' AND ParentID = '.$this->owner->ParentID;
        }
    ...
    public function Settings()
    {
        ...
        if ($this->owner->PauseOnHover) {
            $settings[] = 'data-cycle-pause-on-hover="true"';
        }
        if (!$this->owner->Loop) {
            $settings[] = 'data-cycle-allow-wrap="false"';
        }
        if ($this->owner->SlideEffect) {
            $settings[] = self::$effects[$this->owner->SlideEffect];
        }
